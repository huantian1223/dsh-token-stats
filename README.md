...
- **六张统计卡片**：累计 Token 数、今日 Token（含占累计比例）、峰值 Token（单会话最高）、最长聊天时长、当前/最长连续天数
- **Token 活动热力图**：GitHub 风格 7 行日历网格，支持 每日 / 每周 / 累计 三种视图与 12个月 / 3个月 / 30天 时间范围切换（默认 3 个月），悬停显示当天用量，**点击日期格子下钻查看当天各会话明细（显示会话标题）**，支持导出 CSV（当前范围按日数据）
