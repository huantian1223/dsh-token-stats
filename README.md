...
- **六张统计卡片**：累计 Token 数、今日 Token（含占累计比例）、峰值 Token（单会话最高）、最长聊天时长、当前/最长连续天数；卡片网格自适应列数（宽屏 5-6 列、窄容器自动降列），标题与数字不截断
- **Token 活动热力图**：GitHub 风格 7 行日历网格，支持 每日 / 每周 / 累计 三种视图与 12个月 / 3个月 / 30天 时间范围切换（默认 3 个月），悬停显示当天用量，**点击日期格子下钻查看当天各会话明细（显示会话标题，重复标题自动合并，每页 10 条分页）**，支持导出 CSV（当前范围按日数据）
- **消耗构成**：输入 / 输出 / 缓存读取 / 缓存写入 / 推理，完整精确数字
- **按模型 / 工作区**：各模型、各工作区消耗拆分
- **会话头部实时角标**：当前会话已用 Token，点击弹出本会话明细（显示会话标题，一眼识别是哪个会话），可一键打开完整统计
- **DeepSeek 账户余额**：三处显示——角标右侧余额胶囊（¥）、点击胶囊打开**独立余额弹窗**（总余额/充值/赠送/可用状态/刷新按钮/更新时间）、完整统计页余额面板（含刷新按钮）。点击刷新即强制查询（`?force=1`）并带刷新中反馈，15 分钟自动轮询兜底；API Key 仅存于 host 进程，不下发浏览器
